...
pub struct EncodingTable<E: Ext> {
    pub table: HashMap<u8, OpCodeType>,
    ext: E
}
...
impl<E: Ext> EncodingTable<E> {
    pub fn get(&self, opcode: &OpCode) -> OpCodeType {
        if let Some(op_type) = self.table.get(opcode) {
            return *op_type
        } else {
...
impl Default for EncodingTable<I64> {
    fn default() -> EncodingTable<I64> {
        let mut table: HashMap<u8, OpCodeType> = I32_TABLE.iter().enumerate().map(|(idx, typ)| {
            return (idx as u8, *typ)
        }).collect();

        table.retain(|k, v| *v != OpCodeType::Invalid);
